(function ($) {
	...
	$(document).on('click', 'a[href^="#"]', function (e) {
		...
		if (hash && hash !== '#' && $(hash).length) {
			...
			if (smoother) {
				smoother.scrollTo(hash, true, "top 90");
			} else {
				var targetOffset = $(hash).offset().top - 90;
    ...
    $(".tptesti").on("mouseenter", function () {
        $(".tptesti").removeClass("active");
	...
	// Footer Vastu Vihar text scroll zoom & solid animation
	const footerArea = document.querySelector(".tp-footer-area");
	const footerVastuText = document.querySelector(".footer-vastu-text");
	if (footerArea && footerVastuText) {
		let footerTextLine = gsap.timeline({
			scrollTrigger: {
				trigger: footerArea,
				start: "top 75%",
				end: "bottom bottom",
		...
		footerTextLine.fromTo(footerVastuText, 
			{
				scale: 0.95,
				opacity: 0.35,
